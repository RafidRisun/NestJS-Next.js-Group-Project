        ...
        const fetchUserData = async () => {
            try {
                ...
                if (token) {
                    ...
                    // Fetch feedbacks
                    const faqsResponse = await axios.get('http://localhost:3000/faq/admin', {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
                    });
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 justify-center bg-gray-50 py-5 px-4 sm:px-6 lg:px-8">
                <Toaster />
                {faqs.map((faq) => (
                    <div key={faq.id} className="bg-white shadow-md rounded-md p-4">
                        <h2 className="text-xl font-semibold text-gray-900">FAQ</h2>
                        <p className="text-xl text-black">Subject: {faq.subject}</p>
                        <p className="text-xl text-black">Description: {faq.description}</p>
                    </div>
                ))}
            </div>
